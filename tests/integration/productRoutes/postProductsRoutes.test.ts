...
describe('postProductRoutes', () => {
  ...
  describe('postNewSingleProductRoute - /products/:id', () => {
    it.only('should create a many-to-many', async () => {
      const products_productOptionsId = uuidv4();
      const products_productOptionsBody = {
        id: products_productOptionsId,
        productId: productOne.id,
        productOptionId: productOneOptionOne.id,
      }
      ...
      await knex('products_productOptions').insert({
        ...products_productOptionsBody,
      });

      const products_productOption =
        await knex('products_productOptions')
          .where({ id: products_productOptionsId })
          .first('*');

      expect(products_productOption).toMatchObject(products_productOptionsBody);
    });
